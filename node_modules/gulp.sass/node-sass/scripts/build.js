...
function afterBuild(options) {
  ...
  var install = path.join(__dirname, '..', process.sassBinaryName, 'binding.node');

  mkdir(path.join(__dirname, '..', process.sassBinaryName), function (err) {
...
function testBinary(options) {
  ...
  fs.stat(path.join(__dirname, '..', process.sassBinaryName, 'binding.node'), function (err) {
    if (err) {
      return build(options);
    }
